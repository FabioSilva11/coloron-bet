class Game {
  ...
  constructor() {
    ...
    // Adicionando áudio
    this.backgroundMusic = new Audio('path_to_background_music.mp3'); 
    this.backgroundMusic.loop = true; // Loop do som durante o jogo
  ...
  generateTelegramMessage() {
    ...
    const telegramUrl = "https://t.me/my_bet_link";
    
    window.open(telegramUrl, "TelegramWindow", "width=600,height=300,top=" + top + ",left=" + left);
}
  ...
  * Display score
  ...
  showResult() {
    ...
    // Formatar o score como um valor monetário
    const formattedScore = score.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' });

    $('.stop-game').css('display', 'flex');
    $('.stop-game .final-score').text(formattedScore + ' !'); // Exibir o score formatado
   ...
   * @return {string} grade
   */
  showGrade(score) { if (score > 30) return "Chuck Norris?"; if (score > 25) return "Você é demais!"; if (score > 20) return "Incrível!"; if (score > 15) return "Ótimo!"; if (score > 13) return "Muito bom!"; if (score > 10) return "Bom trabalho!"; if (score > 5) return "Sério?"; return "Fraco..."; }
  ...
  start(money) {
    ...
    $('#score').text(this.score.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })); // Exibe a pontuação formatada
    this.generateSticks();
    this.generateBall();
  ...
  stop() {
    ...
    this.isRunning = 0;
    // Pausar a música de fundo ao parar o jogo
    this.backgroundMusic.pause();
    this.backgroundMusic.currentTime = 0; // Resetar o som para o início
    ...
    $('#sticks .stick').each(function () {
        if ($(this).offset().left < ballPos && $(this).offset().left > ballPos - stickWidth) {
          ...
          if (Color.getColorFromClass($(this)) == Color.getColorFromClass('#ball')) {
            // Se as cores combinam, aumenta a pontuação
        
            let incrementValue; // Inicializa a variável incrementValue
        
            // Define o valor de incremento com base na quantidade de dinheiro, verificando primeiro os valores maiores
            if (money <= 5) {
                incrementValue = 0.3; // Valor decimal a ser adicionado ao score
            } else if (money <= 10) {
                incrementValue = 0.20; // Valor decimal a ser adicionado ao scor
            } else if (money <= 50) {
                incrementValue = 2.30; // Valor decimal a ser adicionado ao score
            } else if (money <= 70) {
                incrementValue = 3.90; // Valor decimal a ser adicionado ao score
            } else if (money <= 100) {
                incrementValue = 5; // Valor decimal a ser adicionado ao score
            }  else if (money <= 150) {
                incrementValue = 8; // Valor decimal a ser adicionado ao score
            }else{
              incrementValue = 10;
            }
        
            score += incrementValue; // Incrementar o score com o valor decimal
